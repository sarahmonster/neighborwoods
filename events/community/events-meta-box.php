...
<div id='eventDetails' class="inside eventForm bubble">
   <?php do_action( 'tribe_events_detail_top', $postId, true ) ?>
	...
	<table cellspacing="0" cellpadding="0" id="EventInfo">
		<tr>
			<td colspan="2" class="tribe_sectionheader"><h4 class="event-time"><?php _e( 'Time &amp; Date', 'tribe-events-community' ); ?></h4></td>
		</tr>
		...
		<tr>
			...
			<td>
				...
				<span class='timeofdayoptions'>
					...
					<select class="chosen" tabindex="<?php $this->tabIndex(); ?>" name='EventStartHour'>
						<?php echo $startHourOptions; ?>
					</select>
					<select class="chosen" tabindex="<?php $this->tabIndex(); ?>" name='EventStartMinute'>
						<?php echo $startMinuteOptions; ?>
					</select>
					<?php if ( !strstr( get_option( 'time_format', TribeDateUtils::TIMEFORMAT ), 'H' ) ) : ?>
						<select class="chosen" tabindex="<?php $this->tabIndex(); ?>" name='EventStartMeridian'>
		...
		<tr>
			...
			<td>
				...
				<span class='timeofdayoptions'>
					<?php _e( '@','tribe-events-community' ); ?>
					<select class="spEventsInput chosen" tabindex="<?php $this->tabIndex(); ?>" name='EventEndHour'>
						<?php echo $endHourOptions; ?>
					</select>
					<select class="chosen" tabindex="<?php $this->tabIndex(); ?>" name='EventEndMinute'>
						<?php echo $endMinuteOptions; ?>
					</select>
					<?php if ( !strstr( get_option( 'time_format', TribeDateUtils::TIMEFORMAT ), 'H' ) ) : ?>
						<select class="chosen" tabindex="<?php $this->tabIndex(); ?>" name='EventEndMeridian'>
	...
	<div class="tribe_sectionheader"><h4><?php _e( 'Location', 'tribe-events-community' ); ?></h4></div>
	<div style="float: left;">
		<table id="event_venue" class="eventtable">
         <?php do_action( 'tribe_venue_table_top', $postId ) ?>
			...
			?>
			<input type="hidden" value="1" name="EventShowMapLink">
			<input type="hidden" value="1" name="EventShowMap">
		</table>
	</div>
   <?php do_action( 'tribe_after_location_details', $postId ); ?>
	<table id="event_organizer" class="eventtable">
			<tr>
				<td colspan="2" class="tribe_sectionheader"><h4><?php _e( 'Organization', 'tribe-events-community' ); ?></h4></td>
			</tr>
